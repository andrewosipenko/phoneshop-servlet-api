...
public class CartServiceImplTest {
    ...
    public void givenNotPresentedCart_whenGetCart_thenGiveCart() {
        ...
        assertNotNull(cart);
        verify(request.getSession()).setAttribute(eq(CartServiceImpl.CART_SESSION_ATTRIBUTE), any(Cart.class));
    }

    @Test
    public void givenPresentedCart_whenGetCart_thenGiveCart() {
        Cart expectedCart = new Cart();
        when(request.getSession().getAttribute(CartServiceImpl.CART_SESSION_ATTRIBUTE)).thenReturn(expectedCart);

        Cart cart = cartService.getCart(request);

        assertSame(expectedCart, cart);
        verify(request.getSession(), never()).setAttribute(eq(CartServiceImpl.CART_SESSION_ATTRIBUTE), any(Cart.class));
    }
