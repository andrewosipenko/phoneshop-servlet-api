...
public class ProductDetailsPageServletTest {
    ...
    public void setup() throws ServletException {
        ...
        when(request.getParameter("quantity")).thenReturn("3").thenReturn("1000");
    ...
    public void testDoPost() throws ServletException,IOException{
        ...
        verify(response).sendRedirect(request.getContextPath()+ "/products/" + 0 +
                "?message=Product was added to cart");
        servlet.doPost(request,response);
        verify(request).setAttribute(eq("error"),anyString());
        verify(request).setAttribute(eq("product"),any(Product.class));
        verify(request).setAttribute(eq("cart"),anyCollection());
