...
public class HttpSessionCartService implements CartService {
    ...
    private void recalculateTotals(Cart cart){
        cart.setTotalQuantity((int) cart.getCartItems().stream().mapToInt(CartItem::getQuantity).sum());
        cart.setTotalCost(BigDecimal.valueOf(cart.getCartItems().stream()
                .mapToInt(item->item.getQuantity()*item.getProduct().getPrice().intValueExact()).sum()));
    }
}
