...
public class ArrayListProductDao implements ProductDao {
    ...
    @Override
    public synchronized Product getProduct(Long id) throws ProductNotFoundExceprion {
        return products.stream()
                .filter(product -> id.equals(product.getId()))
                .findAny()
                .orElseThrow(ProductNotFoundExceprion::new);
    }
    ...
    public synchronized void save(Product product) {
        if (product.getId() != null) {
            Product tmpProduct = products.stream()
                    .filter(item -> product.getId().equals(item.getId()))
                    .findAny()
                    .get();
            products.set(products.indexOf(tmpProduct), product);
        } else {
            product.setId(maxId++);
    ...
    @Override
    public synchronized void delete(Long id) throws ProductNotFoundExceprion {
        Product tmpProduct = products.stream()
