...
public class ProductSampleDataListener implements ServletContextListener {
    ...
    public void contextInitialized(ServletContextEvent servletContextEvent) {
        ...
        String sampleDataParam = "sampleData";
        if (servletContext.getInitParameter(sampleDataParam).equals("enable")) {
            loadSampleData();
        }
    }
    ...
    private List<PriceHistory> addPriceHistory(BigDecimal price) {
        List<PriceHistory> priceHistory = new ArrayList<>();
        Currency usd = Currency.getInstance("USD");
        priceHistory.add(new PriceHistory("15.05.2018", price.add(new BigDecimal(30)), usd));
        priceHistory.add(new PriceHistory("02.10.2018", price.add(new BigDecimal(20)), usd));
        priceHistory.add(new PriceHistory("30.06.2019", price.add(new BigDecimal(10)), usd));
        return priceHistory;
    }
}
